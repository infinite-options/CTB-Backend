        ...
        try:
            ...
            with open(filepath) as csv_file:
                ...
                for items in data[1:]:
                    ...
                    print("\nCurrent Level: ", currentLevel, "Current LFT: ",
                          lft, "Current Row: ", currentRow, type(currentRow))
                    ...
                    # If current level is above next level then find next match
                        ...
                        for remainingItems in data[(currentRow + 1):]:
                            print("Current Level: ", currentLevel, "Next Level: ", int(
                                remainingItems[levelIndex]))
                            if int(remainingItems[levelIndex]) <= currentLevel:
                                print("End Assembly")
                                print(remainingItems[lftIndex])
                                levelDiff = int(
                                    remainingItems[levelIndex]) - currentLevel
                                print(levelDiff)
                                rgt = int(
                                    remainingItems[lftIndex]) - 1 + int(levelDiff)
                                print("RGT is: ", rgt)
                                break
                    ...
                    if (rgt == lft + 1 and PNIndex != -1):
                        ...
                        jsondata.extend([{'Level': int(data[currentRow][levelIndex]), 'PN':data[currentRow]
                                        [PNIndex], 'Qty':data[currentRow][QtyIndex], 'lft':lft, 'rgt':rgt, 'Parent':'Child'}])
                        ...
                        print("in Parent")
                        jsondata.extend([{'Level': int(data[currentRow][levelIndex]), 'PN':data[currentRow]
                                        [PNIndex], 'Qty':data[currentRow][QtyIndex], 'lft':lft, 'rgt':rgt, 'Parent':'Parent'}])
...
class ImportFile(Resource):
    def post(self):
        ...
        try:
            ...
            stream = io.StringIO(
                filepath.stream.read().decode("UTF8"), newline=None)
...
def TraverseTable(file, filename):
    ...
    try:
        ...
        for items in file[1:]:
            ...
            # If current level is above next level then find next match
                ...
                for remainingItems in file[(currentRow + 1):]:
                    print("Current Level: ", currentLevel,
                          "Next Level: ", int(remainingItems[levelIndex]))
                    if int(remainingItems[levelIndex]) <= currentLevel:
                        print("End Assembly")
                        print(remainingItems[lftIndex])
                        levelDiff = int(
                            remainingItems[levelIndex]) - currentLevel
                        print(levelDiff)
                        rgt = int(remainingItems[lftIndex]
                                  ) - 1 + int(levelDiff)
                        print("RGT is: ", rgt)
                        break
            ...
            items[rgtIndex] = rgt
            print("\nCurrent Item: ", items)
            print("Current Data: ", lft, rgt, PNIndex,
                  items[PNIndex], items[QtyIndex])
            ...
            if currentRow == 1:
                ...
                parent_left = last_level_row[str(
                    int(items[levelIndex]) - 1)][1]
                print("Parent: ", parent, parent_left)
            ...
            if (rgt == lft + 1 and PNIndex != -1):
                ...
                tree[parent][parent_left][items[PNIndex]] = {'idx': currentRow - 1, 'lft': lft, 'rgt': rgt, 'qty': items[QtyIndex], 'parent': last_level_row[str(
                    int(items[levelIndex]) - 1)][0], 'parent-lft': last_level_row[str(int(items[levelIndex]) - 1)][1]}

                jsondata.extend([{'Level': int(items[levelIndex]), 'PN':items[PNIndex],
                                'Qty':items[QtyIndex], 'lft':lft, 'rgt':rgt, 'Parent':'Child'}])
                if (items[PNIndex] not in children):
                    children.append(items[PNIndex])
                ...
                if currentRow != 1:
                    ...
                    if items[PNIndex] not in tree:
                        tree[items[PNIndex]] = {}
                    tree[items[PNIndex]][tree_key] = {}
                    last_level_row[items[levelIndex]] = [
                        items[PNIndex], tree_key]
                ...
                jsondata.extend([{'Level': int(items[levelIndex]), 'PN':items[PNIndex],
                                'Qty':items[QtyIndex], 'lft':lft, 'rgt':rgt, 'Parent':'Parent'}])
                if (items[PNIndex] not in parents):
                    parents.append(items[PNIndex])
        ...
        print("-------The productpn $$", productpn)
        print("-------The productpn $$", producttype)
        print("-------The productpn $$", productmfr)
        # Run query to enter new product UID and BOM into table
        productquery = '''
            INSERT INTO pmctb.products
            SET product_uid = \'''' + new_product_uid + '''\',
                ...
                product_manufacturer = \'''' + productmfr + '''\',
                product_BOM = \'''' + jsonBOM + '''\',
                product_parents = \'''' + jsonparents + '''\',
                product_children = \'''' + jsonchildren + '''\',
...
# -----------------------------------------------------------------------
# Imported from local_to_sql.py file. The below code is for generation of data which includes Bom, supplier, inventory, video and parts.
...
#             i= alt_models[int(updatemodel)-1]
        ...
        TraverseTable(list_of_rows,'Home Appliance')
        print(df_BOM)
    return "Null", True
...
class Insertparts(Resource):
    def post(self):
        ...
        try:
            ...
            for v in models:
                print(v)
                alt_models, flag = new_generate_data(v)
...
def CreateInvChildView(parent_product, build_geo):
    try:
        ...
        raise BadRequest(
            'Create Inventory Child View failed, please try again later.')
    finally:
        disconnect(conn)
...
def CreateWorkingCTBView(self):
    try:
        ...
        raise BadRequest(
            'Create WorkingCTB View failed, please try again later.')
    finally:
        disconnect(conn)
...
class Inventory(Resource):
    def get(self):
        ...
        try:
            ...
            raise BadRequest(
                'Inventory Request failed, please try again later.')
        finally:
            disconnect(conn)
...
class GetAllocation(Resource):
    def get(self, product_uid):
        ...
        try:
            ...
            raise BadRequest(
                'Get Allocation Request failed, please try again later.')
        finally:
            disconnect(conn)
...
class AddPartsFromFile(Resource):
    def post(self):
        print("\nInside Add Parts")
        response = {}
        items = {}
        ...
        try:
            ...
            for data in upload_data[1:]:
                ...
                partquery = '''
                    ...
                    SET PN = \'''' + data[0] + '''\',
                        Description = \'''' + data[1] + '''\',
                        Unit_Cost = \'''' + str(data[2]) + '''\',
                        Cost_Unit = \'''' + data[3] + '''\',
                        Weight = \'''' + str(data[4]) + '''\',
                        Weight_Unit = \'''' + data[5] + '''\',
                        Material = \'''' + data[6] + '''\',
                        Vendor = \'''' + data[7] + '''\',
                        Country_of_Origin = \'''' + data[8] + '''\',
                        Lead_Time = \'''' + str(data[9]) + '''\',
                ...
                if parent in tree:
                    if len(tree[parent]) == 1:
                        ...
                        for parent_lft in tree[parent]:
                            need = need_calc_helper_extended(
                                tree, parent, specific=True, pn_left=parent_lft)
                            # update_ctb_list(ctb_list, parent, parent_lft, need)
                            for child_pn in need:
                                for pn_left, qty in need[child_pn].items():
                                    entry = {
...
class RunOrderListVishal(Resource):
    def post(self):
        ...
        try:
            ...
            raise BadRequest(
                'Run Order List Vishal failed, please try again later.')
        finally:
            disconnect(conn)
    ...
    if pn in tree and tree[pn] and pn_left in tree[pn]:
        ...
        for child in tree[pn][pn_left]:
            ...
            if child in tree and child_pn_left in tree[child]:
                req = need_calc_helper_extended(
                    tree, child, specific=True, pn_left=child_pn_left)
                for req_pn in req:
                    ...
                    for req_pn_left in req[req_pn]:
                        ...
                        if req_pn_left in result[req_pn]:
                            ...
                            result[req_pn][req_pn_left] = current + \
                                qty_child * qty
                        else:
...
api.add_resource(AddPartsFromFile, "/api/v2/AddPartsFromFile")
